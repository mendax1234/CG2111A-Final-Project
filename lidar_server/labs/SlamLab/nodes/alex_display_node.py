...
def lidarDisplayProcess(setupBarrier:Barrier=None, readyBarrier:Barrier=None):
    ...
    lidarScanAxis, scanPointsArtist, robotMarker = createLidarPlot(fig, gs)
...
def createLidarPlot(fig:plt.Figure, gs:gridspec.GridSpec):
    ...
    # Add a small circle marker at the origin to represent the robot
    robot_angle = 0  # radians
    robot_radius = 0  # center
    robot_marker, = ax.plot([robot_angle], [robot_radius],
                            marker='o', markersize=30, markeredgecolor='cyan',
    ...
    lidarScanAxis = ax
    return lidarScanAxis, scanPointsArtist, robot_marker
